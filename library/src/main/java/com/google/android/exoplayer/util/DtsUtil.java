...
public final class DtsUtil {
   ...
   */
  public static MediaFormat_vadio parseDtsFormat(byte[] frame, String trackId, long durationUs,
      String language) {
    ...
    int bitrate = rate >= TWICE_BITRATE_KBPS_BY_RATE.length ? MediaFormat_vadio.NO_VALUE
        : TWICE_BITRATE_KBPS_BY_RATE[rate] * 1000 / 2;
    frameBits.skipBits(10); // MIX, DYNF, TIMEF, AUXF, HDCD, EXT_AUDIO_ID, EXT_AUDIO, ASPF
    channelCount += frameBits.readBits(2) > 0 ? 1 : 0; // LFF
    return MediaFormat_vadio.createAudioFormat(trackId, MimeTypes.AUDIO_DTS, bitrate,
